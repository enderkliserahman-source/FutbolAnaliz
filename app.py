...
st.set_page_config(layout="centered", initial_sidebar_state="collapsed", page_title="Futbol İstişare Masası")
...
<style>
    ...
    /* Sekme Başlıkları ve Buton Stili */
    ...
    /* Etiketler ve Girdiler */
    label p { color: #f8fafc !important; font-weight: 600 !important; font-size: 13px !important; }
    div[data-baseweb="select"] > div { background-color: #334155 !important; border: 1px solid #475569 !important; color: #ffffff !important; }
    div[data-baseweb="input"] { background-color: #334155 !important; border: 1px solid #475569 !important; }
...
with tab1:
    st.markdown("<h2 style='color:#ffffff; margin-bottom:0px;'>📋 Günün Bülteni & Canlı İstişare</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color:#94a3b8; font-size:13px;'>🔒 Tercihini ve oranını kilitle, ardından model raporunu açıp maç altındaki canlı sohbetle tartış.</p>", unsafe_allow_html=True)
    ...
    for m in matches:
        ...
        <div class="match-box">
            ...
            <h3 style="margin:4px 0 0 0; color:#ffffff; font-size:17px;">{m['match']}</h3>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.revealed.get(m_id, False):
            st.markdown(f"""
            <div style="background:#0b1329; border:1px solid #1e293b; padding:10px; border-radius:6px; margin-top:8px;">
                <b style="color:#ffffff;">🤖 Model Sinyali:</b> <span style="color:#38bdf8; font-weight:bold;">{m['model_market']}</span><br>
            ...
            if saved_choice != "Seçim Yapılmadı":
                ...
                status_label = "🟢 Tam Mutabakat" if is_match else "🟡 İstişare Gerekli"
                
                st.markdown(f"""
                <div style="margin-top:6px; padding:6px 10px; border-radius:4px; background:{status_color}1a; border-left:3px solid {status_color};">
                    <span style="color:{status_color}; font-weight:bold; font-size:12px;">{status_label}</span><br>
                    <small style="color:#ffffff;">Sen: <b>{saved_choice} (@{st.session_state.user_odds.get(m_id, '-')})</b> &nbsp;|&nbsp; Model: <b>{m['model_market']}</b></small>
                </div>
                """, unsafe_allow_html=True)

        with st.expander("📊 Canlı Saha & İstatistik Masası (Mackolik)"):
            c_stat1, c_stat2, c_stat3 = st.columns(3)
            with c_stat1: st.text_input("Dakika:", key=f"min_{m_id}", placeholder="Örn: 34'")
            with c_stat2: st.text_input("Canlı Skor:", key=f"score_{m_id}", placeholder="Örn: 1-0")
            with c_stat3: st.text_input("Topla Oynama (%):", key=f"poss_{m_id}", placeholder="Örn: 58 - 42")
            
            c_xg1, c_xg2 = st.columns(2)
            with c_xg1: st.number_input("Ev Canlı xG:", min_value=0.0, max_value=10.0, step=0.05, key=f"xgh_{m_id}")
            with c_xg2: st.number_input("Dep Canlı xG:", min_value=0.0, max_value=10.0, step=0.05, key=f"xga_{m_id}")
        ...
        with st.expander("💬 Bu Maç İçin Canlı Tartışma / Sohbet"):
            ...
            for chat in st.session_state.chat_logs[m_id]:
                st.markdown(f"""
                <div style="background:#1e3a8a; padding:6px 10px; border-radius:6px; margin-bottom:4px; font-size:13px; color:#ffffff;">
            ...
            with col_msg:
                user_msg = st.text_input("Taktiksel Görüşünü Yaz:", key=f"chat_in_{m_id}", placeholder="Örn: Baskı arttı...")
            with col_send:
